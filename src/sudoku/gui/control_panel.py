"""ControlPanel — docked side panel with labelled sections, buttons, slider,
and a keyboard shortcut reference table.

Sections (top → bottom)
-----------------------
  PUZZLE    Solve / Load / Reset / New Board
  EDIT      Undo / Redo
  VIEW      Toggle Theme
  Speed     Animated step-rate slider
  SHORTCUTS Key reference table
"""
...
class _Button:
    ...
    def draw(self, surface: pygame.Surface, theme: Theme, font: pygame.font.Font) -> None:
        bg = (
            theme.button_bg_active if self._pressed
            else theme.button_bg_hover if self._hovered
...
class _Slider:
    ...
    def _track(self) -> pygame.Rect:
        ...
        return pygame.Rect(self.rect.left, cy - SLIDER_TRACK_H // 2,
                           self.rect.width, SLIDER_TRACK_H)
    ...
    def draw(self, surface: pygame.Surface, theme: Theme,
        ...
        if tx > track.left:
            pygame.draw.rect(surface, theme.highlight_ring,
                             pygame.Rect(track.left, track.top,
                                         tx - track.left, SLIDER_TRACK_H),
                             border_radius=3)
        ...
        label = ("Speed: instant" if self._value <= 0
                 else f"Speed: {int(self._value)} steps/s")
...
class ControlPanel:
    """Docked side panel: sections, buttons, speed slider, shortcut table.

    Call ``_rebuild_layout()`` whenever ``_rect`` changes (e.g. on window
    resize or divider drag).
    """
    ...
    def __init__(
        ...
        # Built by _rebuild_layout
        self._buttons:        list[_Button]          = []
        self._sections:       list[tuple[str, int]]  = []
        self._slider:         _Slider | None         = None
        self._shortcuts_y:    int                    = 0
        self._slider_value:   float                  = 10.0

        self._rebuild_layout()

    # ------------------------------------------------------------------
    # Layout builder (called on init and on every resize)
    # ------------------------------------------------------------------
    ...
    def _rebuild_layout(self) -> None:
        ...
        w = self._rect.width - PADDING * 2
        y = self._rect.top + PADDING
        ...
        section("PUZZLE")
        button("▶  Solve",          self._on_solve)
        button("📂  Load Puzzle",    self._on_load)
        button("↺  Reset Puzzle",   self._on_reset)
        button("＋  New Random",     self._on_new)
        y += SECTION_GAP
        ...
        # Speed slider — preserve current value across rebuilds
        ...
        # Divider line before shortcuts
        self._divider_y = y
        y += 8

        section("SHORTCUTS")
        self._shortcuts_y = y + LABEL_SIZE + 6
    ...
    def draw(self) -> None:
        ...
        # Section labels
        for label, sy in self._sections:
            lsurf = self._label_font.render(label, True, theme.status_fg)
            self._surface.blit(lsurf, (self._rect.left + PADDING, sy))
        ...
        # Thin horizontal rule before shortcuts
        rx = self._rect.left + PADDING
        rw = self._rect.width - PADDING * 2
        pygame.draw.line(self._surface, theme.grid_line_thin,
                         (rx, self._divider_y), (rx + rw, self._divider_y), 1)
    ...
    def _ensure_fonts(self) -> None:
        ...
        if self._label_font is None:
            self._label_font = pygame.font.SysFont(
                "segoeui,arial,sans-serif", LABEL_SIZE, bold=True)
    ...
    def _draw_shortcuts(self, theme: Theme) -> None:
        ...
        for key_str, action in _SHORTCUTS:
